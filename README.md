...
btdht provides a full Python implementation of the BitTorrent mainline DHT. It
...
```bash
git clone https://github.com/sprooty/btdht
cd btdht
pip install -e .
```
...
```python
from btdht.metadata import get_metadata
...
```python
class CrawlerDHT(btdht.DHT_BASE):
...
Subclass `btdht.DHT_BASE` and override `on_<msg>_query` / `on_<msg>_response`:

```python
class MyDHT(btdht.DHT_BASE):
